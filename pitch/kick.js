(function(){"use strict";var STORE_KEY="goalBonus";var OPENING_CREDITS=1000;var STREAK_STEP=1.85;var READ_CHANCE=0.36;var MAX_KICKS=10;var STAKE_STEP=5;var STAKE_CAP=50000;var FLIGHT_MS=650;var SETTLE_MS=1250;var ZONES=["tl","tr","c","bl","br"];var DIVE_ODDS={tl:0.18,tr:0.18,c:0.28,bl:0.18,br:0.18};var ZONE_POINT={tl:{x:170,y:150},tr:{x:470,y:150},c:{x:320,y:240},bl:{x:170,y:300},br:{x:470,y:300}};var DIVE_TILT={tl:-55,tr:55,c:0,bl:-75,br:75};var BALL_HOME={x:320,y:404};var KEEPER_REACH={x:320,y:260};var creditEl=document.getElementById("goal-credit");var refillBtn=document.getElementById("goal-refill");var betField=document.getElementById("goal-bet");var betDown=document.getElementById("goal-bet-down");var betUp=document.getElementById("goal-bet-up");var kickoffBtn=document.getElementById("goal-kickoff");var bankBtn=document.getElementById("goal-bank");var callEl=document.getElementById("goal-call");var multEl=document.getElementById("goal-mult");var potEl=document.getElementById("goal-pot");var tallyEl=document.getElementById("goal-tally");var keeperEl=document.getElementById("goal-keeper");var ballEl=document.getElementById("goal-ball");var netEl=document.getElementById("goal-net");var targetBtns=[].slice.call(document.querySelectorAll(".goal-target-888c03c1"));if(!creditEl||!refillBtn||!betField||!kickoffBtn||!bankBtn||!callEl||!multEl||!potEl||!tallyEl||!keeperEl||!ballEl||targetBtns.length!==5){return;}
...
function buildTally(){tallyEl.innerHTML="";state.slots=[];for(var i=0;i<MAX_KICKS;i+=1){var slot=document.createElement("li");slot.className="goal-tally-slot-888c03c1";slot.setAttribute("aria-label","Kick "+(i+1)+": pending");tallyEl.appendChild(slot);state.slots.push(slot);}}
...
netEl.classList.remove("is-bulge-888c03c1");void netEl.getBoundingClientRect();netEl.classList.add("is-bulge-888c03c1");}
function snapHome(){ballEl.classList.add("goal-snap-888c03c1");keeperEl.classList.add("goal-snap-888c03c1");ballEl.style.transform="";keeperEl.style.transform="";void ballEl.getBoundingClientRect();ballEl.classList.remove("goal-snap-888c03c1");keeperEl.classList.remove("goal-snap-888c03c1");}
